...
- Possibility to have a single node cluster with or without ingress controller (via Hetzner LB).
...
If you do not want to have an Hetzner LB also deployed, just set `enable_traefik` to false, and that way you can just use NodePort services to expose your apps without paying for a LB (ideal for things like Rancher).

If you are using NodePorts, do not forget to add the additional firewall rules in your kube.tf (see `extra_firewall_rules`) to open these ports. You can do so and apply the changes even after the initial cluster deployment.
