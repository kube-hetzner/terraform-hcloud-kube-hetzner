...
| <a name="swap_enabled"></a> [swap\_enabled](#input\_swap\_enabled) | When this is enabled, swap will be turned on for all nodes | `bool` | `false` | no |
| <a name="swap_size"></a> [swap\_size](#input\_swap\_size) | This dictates the size of the swap file that's created on the nodes, remember the suffix (Can be M or G, examples: 512M, 1024M, 1G, 8G) | `string` | `''` | no |
