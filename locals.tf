  ...
  common_pre_install_k3s_commands = concat(
    ...
    [
      join("\n", [
        ...
        "  # Configure NetworkManager to handle DHCP changes gracefully",
        "  if systemctl is-active --quiet NetworkManager; then",
        "    NM_CONN=$(nmcli -g GENERAL.CONNECTION device show eth1 2>/dev/null | head -1)",
        "    if [ -n \"$NM_CONN\" ]; then",
        "      # Add persistent default route via private network with metric 256",
        "      # Hetzner DHCP no longer provides default routes for internal networks",
        "      # Metric 256 ensures eth1 (private) has lower priority than eth0 (public Ethernet: 100)",
        "      # See also: https://github.com/kube-hetzner/terraform-hcloud-kube-hetzner/issues/1873",
        "      if ! nmcli -g ipv4.routes connection show \"$NM_CONN\" | grep -q \"0.0.0.0/0.*${local.network_gw_ipv4}\"; then",
        "        if ! nmcli connection modify \"$NM_CONN\" +ipv4.routes \"0.0.0.0/0 ${local.network_gw_ipv4} 256\" >/dev/null 2>&1; then",
        "          echo \"Warning: Failed to add default route on eth1. This node may be affected by Hetzner DHCP changes.\" >&2",
        "        fi",
        "      fi",
        "      ",
        "      # Prevent eth1 (private network) from becoming the system default route",
        "      # This ensures public interface (eth0) is the default for external traffic",
        ...
        "      fi",
        "      ",
        "      # Activate the connection to apply all changes",
        "      if ! nmcli connection up \"$NM_CONN\" >/dev/null 2>&1; then",
        "        echo \"Warning: Failed to activate connection on eth1. This node may be affected by Hetzner DHCP changes.\" >&2",
        "      fi",
        "    fi",
        "  fi",
        "fi",
